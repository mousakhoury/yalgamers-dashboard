...
<div class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-end z-50">
  ...
  <div class="flex flex-col items-start justify-between w-full max-w-[728px] h-screen bg-white" in:fly={{ x: 300, duration: 300 }} out:fly={{ x: 300, duration: 300 }}>
    ...
    <div class="flex flex-col items-start gap-[24px] p-[24px] w-full h-full overflow-auto">
      <h5 class="text-[#533E59] text-[16px] font-semibold">Article Details</h5>
      <div class="flex flex-col items-start gap-[16px] w-full">
        ...
        <div class="grid grid-cols-2 w-full gap-[16px]">
          <div class="flex">
            <SelectOption label="Select Network" bind:value={formData.network} options={["Network 1", "Network 2", "Network 3", "Network 4"]} required={true} />
          </div>
          <div class="flex">
            <SelectOption label="Select Token" bind:value={formData.token} options={["Token 1", "Token 2", "Token 3", "Token 4"]} required={true} />
          </div>
          <div class="flex">
            <SelectOption label="Select Status" bind:value={formData.statue} options={["Status 1", "Status 2", "Status 3", "Status 4"]} required={true} />
          </div>
          <div class="flex">
            <SelectOption label="Select Platform" bind:value={formData.platform} options={["Platform 1", "Platform 2", "Platform 3", "Platform 4"]} required={true} />
          </div>
        </div>

        <TextAria label="Description" placeholder="Enter Description" bind:content={formData.description} required={true} />

        <TextInput label="Reading Time" placeholder="Enter Time..." bind:value={formData.subheading} required={true} />

        <MultiSelect label="Select Genres" placeholder="Select one or more geners" {items} bind:selectedOptions={formData.readingTime} required={true} />
      ...
      <div class="flex flex-col items-start gap-[16px] w-full">
        ...
        <UploadImage label="Slider Main Image" ratio="300/400" bind:selectedFile={formData.sliderImage} required={true} />
        <UploadMultipleImages label="Gallery Images" ratio="300/400" bind:selectedFiles={formData.galleryImages} required={true} />
      </div>
