<script>
  ...
  import ArticleFormModal from "$components/Articles/ArticleFormModal.svelte";
  ...
  let data = Array.from({ length: 101 }, (_, index) => ({
    ...
    photo: Placeholder,
    title: "How to play GTA V and stream it.",
  ...
  const tabs = [
    { title: "Active Articles", type: "Published" },
    { title: "Paused Articles", type: "Paused" },
    { title: "Removed Articles", type: "Deleted" },
  ];

  // Update tabTitle reactively based on selectedTab
  $: tabTitle = tabs.find((tab) => tab.type === selectedTab)?.title || "Articles";
  ...
  function openAddModal() {
    selectedItem = { title: "", description: "", createdBy: "", subheading: "", network: "", token: "", statue: "", platform: "", readingTime: "", geners: "", cardImage: "", sliderImage: "", galleryImages: "", lessons: "", website: "", x: "", telegram: "", discord: "", youtube: "", instagram: "", status: "Published" };
    modalMode = "add";
    isModalOpen = true;
  }
...
<div class="flex flex-col items-start gap-[20px] w-full bg-white rounded-[6px]">
  <div class="flex items-start justify-between w-full px-[20px] pt-[20px]">
    ...
    <button on:click={openAddModal} class="flex items-center gap-[8px] py-[10px] pl-[12px] pr-[16px] rounded-[6px] bg-[#FDEB56] hover:bg-[#cabc45]">
      ...
      <p class="text-[#312D10] text-[14px] font-medium">Add New Articles</p>
    </button>
  </div>
...
{#if isModalOpen}
  <ArticleFormModal title={modalMode === "add" ? "Add New Articles" : "Edit Articles"} formData={selectedItem} submitLabel={modalMode === "add" ? "Publish" : "Update"} on:submit={handleFormSubmit} on:close={closeModal} />
{/if}
