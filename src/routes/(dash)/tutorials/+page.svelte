<script>
  ...
  import TutorialFormModal from "$components/Tutorials/TutorialFormModal.svelte";
  ...
  const tabs = [
    { title: "Active Tutorials", type: "Published" },
    { title: "Paused Tutorials", type: "Paused" },
    { title: "Removed Tutorials", type: "Deleted" },
  ];

  // Update tabTitle reactively based on selectedTab
  $: tabTitle = tabs.find((tab) => tab.type === selectedTab)?.title || "Tutorials";
  ...
  function openAddModal() {
    selectedItem = { title: "", description: "", createdBy: "", subheading: "", cardImage: "", sliderImage: "", sliderBgImage: "", lessons: "", website: "", x: "", telegram: "", discord: "", status: "Published" };
    modalMode = "add";
    isModalOpen = true;
  }
...
<div class="flex flex-col items-start gap-[20px] w-full bg-white rounded-[6px]">
  <div class="flex items-start justify-between w-full px-[20px] pt-[20px]">
    ...
    <button on:click={openAddModal} class="flex items-center gap-[8px] py-[10px] pl-[12px] pr-[16px] rounded-[6px] bg-[#FDEB56] hover:bg-[#cabc45]">
      ...
      <p class="text-[#312D10] text-[14px] font-medium">Add New Tutorial</p>
    </button>
  </div>
...
{#if isModalOpen}
  <TutorialFormModal title={modalMode === "add" ? "Add New Tutorial" : "Edit Tutorial"} formData={selectedItem} submitLabel={modalMode === "add" ? "Publish" : "Update"} on:submit={handleFormSubmit} on:close={closeModal} />
{/if}
