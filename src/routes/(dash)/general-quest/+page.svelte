<script>
  ...
  import GeneralFormModal from "$components/GeneralQuests/GeneralFormModal.svelte";

  export let columns = [
    { label: "#", key: "id" },
    { label: "Title", key: "title" },
    { label: "Details", key: "subheading" },
    { label: "Creation Date", key: "creationDate" },
  ];

  let data = Array.from({ length: 30 }, (_, index) => ({
    id: (index + 1).toString().padStart(2, "0"),
    title: "Link your X to get 500XP.",
    subheading: "Discover the latest games on YalGamers! Check...",
  ...
  const tabs = [
    { title: "Active Tasks", type: "Published" },
    { title: "Removed Tasks", type: "Deleted" },
  ];
  ...
  function openAddModal() {
    selectedItem = { title: "", subheading: "", category: "", link: "", status: "Published" };
    modalMode = "add";
    isModalOpen = true;
  }
...
<div class="flex flex-col items-start gap-[20px] w-full bg-white rounded-[6px]">
  <div class="flex items-start justify-between w-full px-[20px] pt-[20px]">
    ...
    <button on:click={openAddModal} class="flex items-center gap-[8px] py-[10px] pl-[12px] pr-[16px] rounded-[6px] bg-[#FDEB56] hover:bg-[#cabc45]">
      ...
      <p class="text-[#312D10] text-[14px] font-medium">Add New Task</p>
    </button>
  </div>
...
{#if isModalOpen}
  <GeneralFormModal title={modalMode === "add" ? "Add New Task" : "Edit Task"} formData={selectedItem} submitLabel={modalMode === "add" ? "Publish" : "Update"} on:submit={handleFormSubmit} on:close={closeModal} />
{/if}
