<script>
  ...
  import NewsFormModal from "$components/news/NewsFormModal.svelte";
  ...
  const tabs = [
    { title: "Active News", type: "Published" },
    { title: "Paused News", type: "Paused" },
    { title: "Removed News", type: "Deleted" },
  ];
  ...
  function openAddModal() {
    selectedItem = { title: "", description: "", createdBy: "", subheading: "", cardImage: "", bannerImage: "", website: "", x: "", telegram: "", discord: "", youtube: "", instagram: "", status: "Published" };
    modalMode = "add";
    isModalOpen = true;
  }
...
<div class="flex flex-col items-start gap-[20px] w-full bg-white rounded-[6px]">
  <div class="flex items-start justify-between w-full px-[20px] pt-[20px]">
    ...
    <button on:click={openAddModal} class="flex items-center gap-[8px] py-[10px] pl-[12px] pr-[16px] rounded-[6px] bg-[#FDEB56] hover:bg-[#cabc45]">
      ...
      <p class="text-[#312D10] text-[14px] font-medium">Add New News</p>
    </button>
  </div>
...
{#if isModalOpen}
  <NewsFormModal title={modalMode === "add" ? "Add New News" : "Edit News"} formData={selectedItem} submitLabel={modalMode === "add" ? "Publish" : "Update"} on:submit={handleFormSubmit} on:close={closeModal} />
{/if}
