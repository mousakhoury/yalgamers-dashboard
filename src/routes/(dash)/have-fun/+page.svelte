<script>
  ...
  import HaveFunFormModal from "$components/HaveFun/HaveFunFormModal.svelte";

  export let columns = [
    { label: "#", key: "id" },
    { label: "Photo", key: "photo" },
    { label: "Name", key: "name" },
    { label: "Launched on", key: "launchedOn" },
    { label: "Total Played", key: "totalPlayed" },
    { label: "Weekly Leaderboard Reward", key: "weeklyReward" },
    { label: "Last Week Winner", key: "lastWinner" },
  ...
  let data = Array.from({ length: 30 }, (_, index) => ({
    ...
    photo: Placeholder,
    name: "Camel Jumping",
    launchedOn: new Date(2024, 0, 12 - index),
    totalPlayed: 553,
    weeklyReward: "500 XP",
    lastWinner: "Ali Haninah",
    status: index % 3 === 0 ? "Published" : index % 3 === 1 ? "Paused" : "Deleted",
  }));
  ...
  const tabs = [
    { title: "Active Games", type: "Published" },
    { title: "Paused Games", type: "Paused" },
    { title: "Removed Games", type: "Deleted" },
  ];

  // Update tabTitle reactively based on selectedTab
  $: tabTitle = tabs.find((tab) => tab.type === selectedTab)?.title || "Games";
  ...
  function openAddModal() {
    selectedItem = { name: "", description: "", subheading: "", xp: "", logo: "", coverPhoto: "", zipFile: "", status: "Published" };
    modalMode = "add";
    isModalOpen = true;
  }
...
<div class="flex flex-col items-start gap-[20px] w-full bg-white rounded-[6px]">
  <div class="flex items-start justify-between w-full px-[20px] pt-[20px]">
    ...
    <button on:click={openAddModal} class="flex items-center gap-[8px] py-[10px] pl-[12px] pr-[16px] rounded-[6px] bg-[#FDEB56] hover:bg-[#cabc45]">
      ...
      <p class="text-[#312D10] text-[14px] font-medium">Add New Game</p>
    </button>
  </div>

  <GamesDataTable {columns} {selectedTab} data={filteredData} {totalResults} {resultsPerPage} {currentPage} {totalPages} on:edit={(e) => openEditModal(e.detail)} on:pageChange={handlePageChange} />
</div>

{#if isModalOpen}
  <HaveFunFormModal title={modalMode === "add" ? "Add New Mini Game" : "Edit Mini Game"} formData={selectedItem} submitLabel={modalMode === "add" ? "Publish" : "Update"} on:submit={handleFormSubmit} on:close={closeModal} />
{/if}
